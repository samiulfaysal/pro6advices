...
$(document).ready(function() {
	...
	function formBuilderViewport(id) {
		...
		if($("#FormBuilderViewport").size() > 0) return;

		var href = $('#' + id).find('a').attr('href');
		var $iframe = $("<iframe frameborder='0' id='FormBuilderViewport'></iframe>").attr('src', href);
		var $note = $("<h2>Loading&hellip;</h2>"); 

		$("#" + id).prepend($iframe).find(".ui-widget-content").remove();
	...
	$("#_ProcessFormBuilderEmbed").click(function() { 
		$.get("../embedForm?id=" + $("#form_id").val(), function(data) {
			...
			$(".ProcessFormBuilderAccordion").accordion({ autoHeight: false });
		}); 
	});
