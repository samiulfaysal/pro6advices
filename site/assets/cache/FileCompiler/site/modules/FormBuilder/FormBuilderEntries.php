...
class FormBuilderEntries extends \ProcessWire\Wire {
	...
	public function ___delete($id) {
		...
		if($result) \ProcessWire\wirermdir($path, true);
	...
	public function ___deleteAll() {
		...
		\ProcessWire\wirermdir($this->getFilesPath(), true); 
	...
	public function getFilesPath($entryID = 0) { 
		$path = \ProcessWire\wire('forms')->getFilesPath() . 'form-' . $this->forms_id . '/';
	...
	public static function _install($db) {
		...
		try {
			if(!$db->query($sql)) \ProcessWire\wire('modules')->error($this->db->error);
		} catch(Exception $e) { }
	}
