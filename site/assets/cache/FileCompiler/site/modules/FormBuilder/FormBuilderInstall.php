...
class FormBuilderInstall extends \ProcessWire\Wire {

	protected $permissionArray = array(
		'form-builder' => 'Access Form Builder admin page',
		'form-builder-add' => 'Add new Form Builder forms',
		);
	...
	public function install() {
		...
		$template->noGlobal = 1;
		$template->flags = \ProcessWire\Template::flagSystem;
		$template->save();
		...
		$src = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/FormBuilder/FormBuilderInstall.php') . '/' . FormBuilderMain::name . '.php'; 
		$dst = \ProcessWire\wire('config')->paths->templates . FormBuilderMain::name . '.php';
	...
	public function uninstall() {
		...
		$page = \ProcessWire\wire('pages')->get("template=" . FormBuilderMain::name); 
		if($page->id) {
			$this->message("Removing page: {$page->path}"); 
			$page->delete();
		}

		$template = \ProcessWire\wire('templates')->get(FormBuilderMain::name); 
		if($template) { 
			$template->flags = \ProcessWire\Template::flagSystemOverride; 
			$template->flags = 0; 
			$this->message("Removing template: {$template->name}"); 
			\ProcessWire\wire('templates')->delete($template); 
		}

		$fieldgroup = \ProcessWire\wire('fieldgroups')->get(FormBuilderMain::name); 
		if($fieldgroup) {
			$this->message("Removing fieldgroup: {$fieldgroup->name}"); 
			\ProcessWire\wire('fieldgroups')->delete($fieldgroup); 
		}

		$templateFile = \ProcessWire\wire('config')->paths->templates . FormBuilderMain::name . '.php';
