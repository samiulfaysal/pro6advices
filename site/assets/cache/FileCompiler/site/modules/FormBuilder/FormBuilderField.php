...
class FormBuilderField extends FormBuilderData {
	...
	public function set($key, $value) {
		if($key == 'name') $value = preg_replace('/[^-_.a-zA-Z0-9]/', '_', $value); 
	...
	public function getArray() {
		...
		// check if this field is a container for other \ProcessWire\fields(children)
	...
	public function getChildrenFlat($level = 0) {
		...
		foreach($this->children as $key => $child) {
			...
			if($numChildren || $child->type == 'Fieldset') {
				...
				// append the children
				if($numChildren) foreach($child->getChildrenFlat($level+1) as $c) $children[$c->name] = $c; 

				// close the fieldset
				$end = new FormBuilderField();
				$end->name = $child->name . '_END';
				$end->type = '';
				$end->level = $level;
				$children[$end->name] = $end; 
			}
