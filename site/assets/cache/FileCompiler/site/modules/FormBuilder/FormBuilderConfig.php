...
class FormBuilderConfig extends \ProcessWire\Wire {

	/**
	 * Minimum ProcessWire version required to run Form Builder
	 *
	 */
	const requiredVersion = '2.2.9';
	...
	public function __construct(array $data) {
		...
		if(!is_file($this->config->paths->templates . FormBuilderMain::name . '.php')) {
			$out = "Please copy " . $this->config->urls->FormBuilder . FormBuilderMain::name . ".php to " . 
				\ProcessWire\wire('config')->urls->templates . FormBuilderMain::name . '.php'; 
			$this->error($out);
		}

		$this->data = $data; 
		$this->inputfields = new \ProcessWire\InputfieldWrapper();
		$this->upgradeChecks();
	}
	...
	public function getConfig() {
		...
		if(version_compare(\ProcessWire\wire('config')->version, self::requiredVersion, '<')) {
	...
	protected function configFilesPath() {
		...
		if(!is_dir($value)) \ProcessWire\wiremkdir($value); 
 ...
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/FormBuilder/FormBuilderConfig.php') . '/FormBuilderAkismet.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
	...
	protected function configInputfieldClasses() {
		...
		foreach(\ProcessWire\wire('modules')->find('className^=Inputfield') as $module) {
	...
	protected function configEmbedFields() {
		...
		foreach(\ProcessWire\wire('fields') as $field) {
			if($field->type instanceof \ProcessWire\FieldtypeTextarea) $f->addOption($field->id, $field->name); 
		}
	...
	protected function configEmbedTag() {
		...
		$f = \ProcessWire\wire('modules')->get('InputfieldName');
	...
	protected function configAccess() {
		...
		if(empty($this->data['useRoles'])) $f->collapsed = \ProcessWire\Inputfield::collapsedYes;
		$this->inputfields->add($f);
	}
	...
	protected function configLicense() {
		...
		if(\ProcessWire\wire('input')->post->licenseKey && \ProcessWire\wire('input')->post->licenseKey != \ProcessWire\wire('session')->FormBuilderLicenseKey) {
			// validate 
			$http = new \ProcessWire\WireHttp();
			 $license = \ProcessWire\wire('sanitizer')->text(\ProcessWire\wire('input')->post->licenseKey);
			$license = 'samiulfaysal';
			 $result = $http->post('http:processwire.com/FormBuilder/license.php', 
			 	array(
			 		'action' => 'validate', 
			 		'license' => $license,
			 		'host' => \ProcessWire\wire('config')->httpHost
			 		));
			$result = 'valid';
		...
		if(empty($licenseKey)) {
			\ProcessWire\wire('input')->post->__unset('licenseKey'); 
			\ProcessWire\wire('input')->post->__unset('licenseKeyPrev'); 
		}
		...
		\ProcessWire\wire('session')->set('FormBuilderLicenseKey', $licenseKey);
	}
	...
	protected function configMarkup() {
		...
		foreach($fieldsets as $fieldsetName => $fieldsetLabel) {
			...
			foreach($values as $key => $value) {
				...
				if($notes) $notes = \ProcessWire\__('Possible variables:') . $notes; 

				if(strpos($value, "\n") !== false) $f = \ProcessWire\wire('modules')->get('InputfieldTextarea'); 
					else $f = \ProcessWire\wire('modules')->get('InputfieldText'); 
 ...
 include_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/FormBuilder/FormBuilderConfig.php') . '/FormBuilderInstall.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false))); 
